...
def Levenshtein_Automata_Dico(refString, levenshtein_distance):
    # Creation des etats de l'automate 
    dict_levenshtein_states = {}
    for j in range(len(refString) + 1):
        for i in range(levenshtein_distance + 1):
            state_label = str(j) + ";" + str(i)
            print("state_label", state_label)
            dict_levenshtein_states[state_label] = automate.add_state()
            print("dict_levenshtein_states[state_label]", dict_levenshtein_states[state_label])
    ...
    for c in refString:
        automata_voc.append(c)

    for state_label, state in dict_levenshtein_states.iteritems():
        print('state', state)
        nb_consummed_chars = state_label.split(";")[0] # 1er caractere du label
        nb_elementary_operations = state_label.split(";")[1] # 2nd caractere du label


        charFromRefStr = refString[int(nb_consummed_chars) - 1]
        ...
        diag_dst_label = [str(int(nb_consummed_chars) + 1) + ";" + str(nb_elementary_operations)]
        # print("diag", diag_dst_label)
        right_dst_label = [str(int(nb_consummed_chars) + 1) + ";" + str(int(nb_elementary_operations) + 1)]
